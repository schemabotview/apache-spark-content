  ...
  sections: [
    ...
    },
    {
      ...
      slide: {
        ...
        body: [
          'Write the rollup as the **accurate batch view** — partitioned so downstream reads prune to just what they need.',
          '',
          '```python',
          '(rollup.write',
          '    .mode("overwrite")',
          '    .partitionBy("day")                  # one folder per day',
          '    .parquet("s3://lake/views/revenue"))',
          '```',
          '',
          '### What’s happening',
          '- `partitionBy("day")` writes **one folder per day** — later reads prune by date',
          '- Parquet keeps the view **columnar** and compressed for fast serving reads',
          '- This is the **source of truth** — accurate, but only as fresh as the last nightly run',
          '',
          '**Exercises:** partitioning & data layout · file formats (gaps: partitioning, sources)',
        ].join('\n'),
      },
      beats: [
        {
          line: 'The final batch step writes our rollup out as the batch view — the trusted, accurate table the serving layer will read. How we write it matters as much as what we write. We partition the output by day, which physically lays the data out as one folder per day on storage. That pays off every time something reads this view later: a query for a particular date only opens that date’s folder and prunes all the others — the same partition pruning we exploited on the way in, now built into our own output. We write it as Parquet so it stays columnar and compressed, keeping those serving reads fast. And we overwrite, so each nightly run produces a clean, correct snapshot. Now, step back and see what the batch layer gives us and what it doesn’t. It gives us numbers we can completely trust — a full, deduplicated, correctly aggregated view of history. But it has one built-in limitation: it’s only as fresh as the last time the nightly job ran. If it ran at 2 a.m., then by mid-afternoon it knows nothing about the last twelve hours of activity. For a dashboard that’s supposed to say “revenue up to now,” that staleness is a real gap. And filling that gap — covering what’s happened since the last batch run — is exactly the job of the speed layer, which we build next.',
